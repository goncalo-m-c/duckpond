...
from duckpond.accounts.quotas import (
    AccountQueryLimiter,
...
def mock_account() -> Account:
    ...
    account.max_storage_gb = 10  # 10 GB limit
    account.max_query_memory_gb = 4  # 4 GB memory limit
    account.max_concurrent_queries = 5  # 5 concurrent queries
    return account
...
@pytest.fixture
def query_limiter() -> AccountQueryLimiter:
    """Create a query limiter for testing."""
    return AccountQueryLimiter()
...
class TestStorageQuota:
    ...
    async def test_check_storage_quota_within_limit(
        self, mock_account: Account, mock_storage_backend: MockStorageBackend
    ) -> None:
        """Test that storage quota check passes when within limit."""
        # Set current usage to 5 GB
        mock_storage_backend.add_file(
            "data/file1.parquet", mock_account.account_id, 5 * 1024 * 1024 * 1024
        )
        ...
        # Should not raise
        await check_storage_quota(mock_account, additional_bytes, mock_storage_backend)
    ...
    async def test_check_storage_quota_exceeds_limit(
        ...
        mock_storage_backend.add_file(
            "data/file1.parquet", mock_account.account_id, 8 * 1024 * 1024 * 1024
        )
        ...
        assert exc_info.value.context["account_id"] == mock_account.account_id
    ...
    async def test_check_storage_quota_exactly_at_limit(
        ...
        mock_storage_backend.add_file(
            "data/file1.parquet", mock_account.account_id, 9 * 1024 * 1024 * 1024
        )
    ...
    async def test_calculate_storage_usage(
        ...
        # Add multiple files
        mock_storage_backend.add_file("data/file1.parquet", account_id, 1024 * 1024 * 1024)  # 1 GB
        mock_storage_backend.add_file("data/file2.parquet", account_id, 512 * 1024 * 1024)  # 512 MB
        mock_storage_backend.add_file("data/file3.csv", account_id, 256 * 1024 * 1024)  # 256 MB

        # Calculate total usage
        usage = await calculate_storage_usage(account_id, mock_storage_backend)
    ...
    async def test_calculate_storage_usage_empty(
        ...
        """Test storage usage calculation for empty account."""
...
class TestConcurrentQueryLimits:
    ...
    async def test_acquire_query_slot_success(
        self, mock_account: Account, query_limiter: AccountQueryLimiter
    ) -> None:
        """Test successful query slot acquisition."""
        async with query_limiter.acquire_query_slot(
            mock_account.account_id, mock_account.max_concurrent_queries
        ):
            # Should successfully acquire slot
            assert query_limiter.get_active_queries(mock_account.account_id) == 1

        # After context exit, should be released
        assert query_limiter.get_active_queries(mock_account.account_id) == 0
    ...
    async def test_acquire_multiple_query_slots(
        ...
        assert query_limiter.get_active_queries(mock_account.account_id) == 3
    ...
    async def test_acquire_query_slot_exceeds_limit(
        ...
        assert query_limiter.get_active_queries(mock_account.account_id) == 5
    ...
    @pytest.mark.asyncio
    async def test_query_limiter_per_account_isolation(
        self, query_limiter: AccountQueryLimiter
    ) -> None:
        """Test that query limits are isolated per account."""
        account1_id = "account-1"
        account2_id = "account-2"

        # Acquire slot for account 1
        async with query_limiter.acquire_query_slot(account1_id, 5):
            assert query_limiter.get_active_queries(account1_id) == 1
            assert query_limiter.get_active_queries(account2_id) == 0

            # Acquire slot for account 2
            async with query_limiter.acquire_query_slot(account2_id, 5):
                assert query_limiter.get_active_queries(account1_id) == 1
                assert query_limiter.get_active_queries(account2_id) == 1

            # Account 2 released
            assert query_limiter.get_active_queries(account2_id) == 0

        # Both released
        assert query_limiter.get_active_queries(account1_id) == 0
        assert query_limiter.get_active_queries(account2_id) == 0

    def test_clear_account(
        self, mock_account: Account, query_limiter: AccountQueryLimiter
    ) -> None:
        """Test clearing account semaphore."""
        # Create semaphore by getting active queries
        query_limiter._get_or_create_semaphore(mock_account.account_id, 5)
        assert mock_account.account_id in query_limiter._semaphores

        # Clear account
        query_limiter.clear_account(mock_account.account_id)

        # Should be removed
        assert mock_account.account_id not in query_limiter._semaphores
        assert mock_account.account_id not in query_limiter._active_queries


class TestDuckDBConnection:
    """Tests for DuckDB connection creation with memory limits."""

    def test_create_account_connection_in_memory(self, mock_account: Account) -> None:
        """Test creating in-memory DuckDB connection with memory limits."""
        conn = create_account_connection(mock_account)
    ...
    def test_create_account_connection_with_path(
        self, mock_account: Account, tmp_path
    ) -> None:
        """Test creating DuckDB connection with file path."""
        db_path = tmp_path / "test.duckdb"

        conn = create_account_connection(mock_account, str(db_path))
...
class TestQuotaUsage:
    ...
    async def test_get_quota_usage(
        ...
        query_limiter: AccountQueryLimiter,
    ) -> None:
        """Test getting quota usage statistics."""
        # Add some storage usage
        mock_storage_backend.add_file(
            "data/file1.parquet", mock_account.account_id, 3 * 1024 * 1024 * 1024  # 3 GB
        )
        ...
        # Get quota usage
        usage = await get_quota_usage(mock_account, mock_storage_backend, query_limiter)

        # Verify statistics
        assert usage.account_id == mock_account.account_id
    ...
    async def test_quota_usage_to_dict(
        ...
        mock_storage_backend.add_file(
            "data/file1.parquet", mock_account.account_id, 7 * 1024 * 1024 * 1024
        )
        ...
        # Verify structure
        assert "account_id" in usage_dict
    ...
    async def test_quota_usage_storage_exceeded(
        ...
        mock_storage_backend.add_file(
            "data/file1.parquet", mock_account.account_id, 12 * 1024 * 1024 * 1024
        )
